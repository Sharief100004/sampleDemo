import { TreeViewModule } from '@syncfusion/ej2-angular-navigations'; // Import the module
...
export class DashboardComponent implements OnInit {
  ...
  public getConsultantName(value: ResourceDetails): string {
    if (value && value.resource && value.resourceData && value.resource.textField) {
      return value.resourceData[value.resource.textField] as string;
    }
    return ''; // Handle the case when the properties are not defined
  }
      ...
      if (scheduleElement) {
        ...
        if (event.target.classList.contains('e-work-cells')) {
          ...
          if (cellData.groupIndex !== undefined) {
            ...
            // Process resourceDetails
            this.resourceDetails = resourceDetails;
          } else {
            console.log("groupIndex is undefined.");
          }
          // const resourceDetails: ResourceDetails = this.scheduleObj.getResourcesByIndex(cellData.groupIndex);
          const eventData: Record<string, any> = {
            ...
            DepartmentID: this.resourceDetails.resourceData['GroupId'],
            ConsultantID: this.resourceDetails.resourceData['Id']
          };
